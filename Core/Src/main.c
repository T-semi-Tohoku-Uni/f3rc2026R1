...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  if (htim == &htim6)
  {
    ...
    for (int h = 0; h < 2; h++)
    {
      ...
      float output =
          motors[h].Kp * error + motors[h].Ki * motors[h].gosagoukei + motors[h].Kd * derivative;
    ...
    if (motors[2].encoder_initialized != 0U)
    {
      ...
      float motor2_position_error =
          (float)(motors[2].target_position_count -
                  motors[2].position_count);
  ...
  for (int i = 0; i < 4; i++)
  {
    if (RxHeader.Identifier == motors[i].can_id)
    {
      ...
      uint16_t new_encoder =
          (uint16_t)(((uint16_t)RxData[0] << 8) |
                     RxData[1]);

      // Byte2,3：現在速度
      motors[i].v =
          (int16_t)(((uint16_t)RxData[2] << 8) |
                    RxData[3]);
      ...
      if (i == 2)
      {
        ...
        if (motors[2].encoder_initialized == 0U)
        {
          ...
          if (HAL_GPIO_ReadPin(
                  motor2_photointerrupter_GPIO_Port,
                  motor2_photointerrupter_Pin) == GPIO_PIN_RESET)
          {
            /*
             * すでに仕切り位置にいるため、動かさない
             */
...
static void MX_GPIO_Init(void)
{
  ...
  GPIO_InitStruct.Pin = motor0_upper_Pin | motor0_lower_Pin | motor1_upper_Pin | motor1_lower_Pin | motor3_upper_Pin | motor3_lower_Pin | motor2_photointerrupter_Pin;
